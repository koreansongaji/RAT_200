...
namespace DinoFracture
{
    ...
    public sealed class AsyncFractureResult
    {
        ...
        private void FirePieceCallbacks(string callbackName)
        {
            if (_args.OriginalObject != null)
            {
                ...
                else if (callbackName == "OnFracture")
                {
    ...
    public sealed class FractureEngine : FractureEngineBase
    {
        ...
        private int _numSyncFracturesThisFrame = 0;
        ...
        internal static new FractureEngine Instance
        {
            get
            {
                if (_instance == null)
        ...
        /// <summary>
        /// If true, cached data used to speed up fracturing will be cleared when
        /// pre-fracturing inside the editor in edit mode. This helps reduce memory
        /// pressure after fracturing.
        /// </summary>
        /// <remarks>
        /// This value has no effect during play mode nor in builds; it will reset after the editor is closed.
        /// </remarks>
        public static bool ClearCacheDataInEditMode
        {
            get
            {
#if UNITY_EDITOR
                return SessionState.GetBool("DinoFracture.ClearCacheDataInEditMode", true);
#else
                return true;
#endif
            }
            ...
            {
#if UNITY_EDITOR
                SessionState.SetBool("DinoFracture.ClearCacheDataInEditMode", value);
#endif
            }
        }
        ...
        /// <summary>
        /// If true, cached data used to speed up fracturing will be cleared when
        /// toggling play mode in the editor. This will reflect the state of a
        /// build upon startup.
        /// </summary>
        /// <remarks>
        /// This value has no effect in builds; it will reset after the editor is closed.
        /// </remarks>
        public static bool ClearCacheDataTogglingPlayMode
        {
            get
            {
#if UNITY_EDITOR
                return SessionState.GetBool("DinoFracture.ClearCacheDataTogglingPlayMode", true);
#else
                return true;
#endif
            }
            ...
            {
#if UNITY_EDITOR
                SessionState.SetBool("DinoFracture.ClearCacheDataTogglingPlayMode", value);
#endif
            }
        }
        ...
        public static AsyncFractureResult StartFracture(FractureDetails details, FractureGeometry callback, Transform piecesParent, bool transferMass, bool hideAfterFracture)
        {
            ...
            else
            {
                ...
                if (FractureBuilder.DisableMultithreading || !details.Asynchronous)
                {
                    if (++Instance._numSyncFracturesThisFrame > EffectiveMaxRunningFractures)
                    {
                        Instance.EnableEditorUpdatePump(true);
                ...
                else
                {
                    ...
                    {
                        Instance.Update();
                        Instance.DestroyGameObjectIfNoRunningFractures();
                    }
#endif
                }
            }
        ...
        private void EnableEditorUpdatePump(bool enable)
        {
#if UNITY_EDITOR
            // Never allow us to hook update twice.
            // This does nothing if not already hooked.
        ...
        private void OnEditorUpdate()
        {
            ...
            DestroyGameObjectIfNoRunningFractures();
        }

        private void DestroyGameObjectIfNoRunningFractures()
        {
            if (_runningFractures.Count == 0)
            {
                EnableEditorUpdatePump(false);
        ...
        private void UpdateFractures()
        {
            ...
            StartPendingFractures();
        }

        private void StartPendingFractures()
        {
            while (_pendingFractures.Count > 0 && (EffectiveMaxRunningFractures <= 0 || _runningFractures.Count < EffectiveMaxRunningFractures))
            {
                _runningFractures.Add(_pendingFractures[0]);
                _pendingFractures.RemoveAt(0);
            }
        }
