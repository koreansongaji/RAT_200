...
namespace DinoFracture
{
    ...
    public class FractureOnCollision : MonoBehaviour
    {
        ...
        /// </summary>
        /// <remarks>
        /// Recommended to set to true for consistency. Set to false for
        ...
        private void Reset()
        {
            // Called when adding the component in the editor.
            // Not called when loading the project.
            // We use this to set our desired values without affecting
            // those who are upgrading to the latest version.
            ConsistentImpactForce = true;
        }
        ...
        internal bool GatherCollisionInfo(Collision col)
        {
            if (!_fireFracture && col.contactCount > 0 && (CollidableLayers.value & (1 << col.gameObject.layer)) != 0)
            {
                ...
                else
                {
                    ...
                    _thisVelocity = (_thisBody != null) ? _thisBody.linearVelocity : Vector3.zero;
#else
...
#if UNITY_6000_0_OR_NEWER
                    desiredVelocity += _thisVelocity - ((_thisBody != null) ? _thisBody.linearVelocity : Vector3.zero);
#else
                ...
                if (AdjustForKinematic)
                {
                    ...
                    if (_thisBody != null && _thisBody.isKinematic && _impactBody != null && _impactMass > 0.0f)
                    {
                        ...
                        else
                        {
                            Vector3 desiredVelocity = -_impactImpulse / _impactMass;
#if UNITY_6000_0_OR_NEWER
                            desiredVelocity += _impactVelocity - _impactBody.linearVelocity;
#else
